...
void Scale::set_total_tick_count(uint32_t total_tick_count) {
  if (obj_)
    lv_scale_set_total_tick_count(obj_, total_tick_count);
}

void Scale::set_major_tick_every(uint32_t major_tick_every) {
  if (obj_)
    lv_scale_set_major_tick_every(obj_, major_tick_every);
}

void Scale::set_label_show(bool show_label) {
  if (obj_)
    lv_scale_set_label_show(obj_, show_label);
}
...
void Scale::set_angle_range(uint32_t angle_range) {
  if (obj_)
    lv_scale_set_angle_range(obj_, angle_range);
}
