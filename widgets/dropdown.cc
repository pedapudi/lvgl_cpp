...
void Dropdown::set_selected(uint32_t sel_opt) {
  if (obj_)
    lv_dropdown_set_selected(obj_, sel_opt);
}
...
void Dropdown::set_symbol(const void* symbol) {
  if (obj_)
    lv_dropdown_set_symbol(obj_, symbol);
}

void Dropdown::set_selected_highlight(bool en) {
  if (obj_)
    lv_dropdown_set_selected_highlight(obj_, en);
}
...
void Dropdown::get_selected_str(char* buf, uint32_t buf_size) {
  if (obj_)
    lv_dropdown_get_selected_str(obj_, buf, buf_size);
}
