...
void Spinbox::set_rollover(bool rollover) {
  if (obj_)
    lv_spinbox_set_rollover(obj_, rollover);
}
...
void Spinbox::set_digit_count(uint32_t digit_count) {
  if (obj_)
    lv_spinbox_set_digit_count(obj_, digit_count);
}

void Spinbox::set_dec_point_pos(uint32_t dec_point_pos) {
  if (obj_)
    lv_spinbox_set_dec_point_pos(obj_, dec_point_pos);
}

void Spinbox::set_step(uint32_t step) {
  if (obj_)
    lv_spinbox_set_step(obj_, step);
}

void Spinbox::set_range(int32_t min_value, int32_t max_value) {
  if (obj_)
    lv_spinbox_set_range(obj_, min_value, max_value);
}

void Spinbox::set_min_value(int32_t min_value) {
  if (obj_)
    lv_spinbox_set_min_value(obj_, min_value);
}

void Spinbox::set_max_value(int32_t max_value) {
  if (obj_)
    lv_spinbox_set_max_value(obj_, max_value);
}
...
void Spinbox::set_digit_step_direction(lv_dir_t direction) {
  if (obj_)
    lv_spinbox_set_digit_step_direction(obj_, direction);
}
...
void Spinbox::step_next() {
  if (obj_)
    lv_spinbox_step_next(obj_);
}

void Spinbox::step_prev() {
  if (obj_)
    lv_spinbox_step_prev(obj_);
}
