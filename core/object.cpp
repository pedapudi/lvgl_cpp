...
void Object::align(Align align_type, int32_t x_ofs, int32_t y_ofs) {
  if (obj_)
    lv_obj_align(obj_, static_cast<lv_align_t>(align_type), x_ofs, y_ofs);
}

void Object::align_to(const Object& base, Align align_type, int32_t x_ofs,
                      int32_t y_ofs) {
  if (obj_ && base.raw())
    lv_obj_align_to(obj_, base.raw(), static_cast<lv_align_t>(align_type),
                    x_ofs, y_ofs);
}

void Object::center() { align(Align::Center, 0, 0); }
